...
links.forEach(link => {
    link.addEventListener('click', function(e) {
        ...
        if (targetId === 'contador') {
            ...
            // Para os outros links, rola suavemente para a seção e centraliza
            targetElement.scrollIntoView({
                behavior: 'smooth',
                block: 'start' // Faz com que a seção comece a rolar para o topo
            });

            // Ajusta a rolagem para centralizar a seção 150px acima do centro
            setTimeout(() => {
                const offset = (window.innerHeight - targetElement.offsetHeight) / 2 - 150; // Subtrai 150px do centro
                window.scrollBy(0, -offset); // Ajusta para centralizar com 150px acima
            }, 300); // Aguarda a rolagem suave antes de ajustar
        }
    });
});
